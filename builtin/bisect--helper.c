...
static void bisect_print_status(const struct bisect_terms *terms)
{
	...
	if (!bs.nr_good && !bs.nr_bad)
		printf(_("info: waiting for both good and bad commits\n"));
	else if (bs.nr_good)
		printf(Q_("info: waiting for bad commit, %d good commit known\n",
			  "info: waiting for bad commit, %d good commits known\n",
			  bs.nr_good), bs.nr_good);
	else
		printf(_("info: waiting for good commit(s), bad commit known\n"));
}
